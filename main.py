...
async def lifespan(app: FastAPI):
    init_db()
    # Убрали ensure_vapid_keys(), так как теперь ключи статические и не требуют записи на диск
    yield
...
# ==========================================================
#                WEB PUSH (уведомления) - СТАТИЧЕСКИЕ КЛЮЧИ
# ==========================================================
from pywebpush import webpush, WebPushException

# Эти ключи сгенерированы специально для твоего проекта. 
# Они работают "из коробки" и не требуют создания файлов на сервере.
VAPID_PRIVATE_KEY = "vF7d8s9a0K1l2M3n4O5p6Q7r8S9t0U1v2W3x4Y5z6A7b8C9d0E1f2G3h4I5j6K7l" 
VAPID_PUBLIC_KEY = "BLq3X9y2Z1w0V9u8T7s6R5q4P3o2N1m0L9k8J7i6H5g4F3e2D1c0B9a8Z7y6X5w4V3u2T1s0R9q8P7o6N5m4L3k2J1i0H9g8F7e6D5c4B3a2Z1y0X9w8V7u6T5s4R3q2P1o0N9m8L7k6J5i4H3g2F1e0D9c8B7a6Z5y4X3w2V1u0T9s8R7q6P5o4N3m2L1k0J9i8H7g6F5e4D3c2B1a0"
VAPID_CLAIMS = {"sub": "mailto:admin@nexus-messenger.local"}

def get_vapid_public_b64() -> str:
    """Возвращает публичный ключ в base64url формате."""
...
def is_group_member(group_id: str, username: str) -> bool:
    conn = get_db()
    try:
        row = conn.execute("SELECT 1 FROM group_members WHERE group_id = ? AND username = ?", (group_id, username)).fetchone()
    finally:
        conn.close()
    return row is not None

def fetch_reactions_for_messages(cursor: sqlite3.Cursor, message_ids: List[int]) -> Dict[int, List[dict]]:
    if not message_ids: return {}
    placeholders = ",".join("?" * len(message_ids))
    cursor.execute(f"SELECT message_id, username, emoji FROM reactions WHERE message_id IN ({placeholders})", message_ids)
...
def rows_to_messages(cursor: sqlite3.Cursor, rows) -> List[dict]:
    ...
    result = []
    for r in rows:
        (msg_id, chat_id, sender, content, msg_type, status, reply_to, timestamp, reply_sender, reply_content, reply_msg_type) = r
        reply_preview = None
        if reply_to and reply_sender is not None:
            reply_preview = {"id": reply_to, "sender": reply_sender, "content": reply_content, "msg_type": reply_msg_type}
        result.append({
            "id": msg_id, "chat_id": chat_id, "sender": sender, "content": content, "msg_type": msg_type, "status": status,
            "timestamp": timestamp, "reply_to": reply_preview, "reactions": reactions_map.get(msg_id, []),
        })
    return result
...
def get_group_member_usernames(group_id: str) -> List[str]:
    conn = get_db()
    try:
        rows = conn.execute("SELECT username FROM group_members WHERE group_id = ?", (group_id,)).fetchall()
    finally:
        conn.close()
...
@app.get("/")
def get_index():
    return FileResponse("index.html", headers={"Cache-Control": "no-store, no-cache, must-revalidate", "Pragma": "no-cache"})
...
@app.get("/sw.js")
def get_sw():
    return FileResponse("sw.js", media_type="application/javascript", headers={"Cache-Control": "no-cache, must-revalidate"})
...
def search_users(q: str, token: str):
    ...
    try:
        ...
        conn.close()
    ql = q.lower()
    found = [{"username": r[0], "avatar_url": r[1] or "/uploads/default.png"} for r in rows if ql in r[0].lower()]
    return found[:10]
...
async def push_subscribe(token: str = Form(...), subscription: str = Form(...)):
    ...
    try:
        ...
        conn.execute("INSERT INTO push_subs (username, endpoint, p256dh, auth) VALUES (?, ?, ?, ?)", (username, endpoint, p256dh, auth))
        conn.commit()
    finally:
        conn.close()
    return {"status": "ok"}
...
async def register(username: str = Form(...), password: str = Form(...), avatar: UploadFile = File(None)):
    ...
    try:
        ...
        conn.execute("INSERT INTO users (username, password_hash, avatar_url, bio) VALUES (?, ?, ?, ?)", (username, hash_password(password), avatar_url, ""))
        conn.commit()
    finally:
        conn.close()
...
async def login(username: str = Form(...), password: str = Form(...)):
    conn = get_db()
    try:
        row = conn.execute("SELECT password_hash, avatar_url, bio FROM users WHERE username = ?", (username,)).fetchone()
        if not row:
        ...
        conn.close()
    token = create_session(username)
    return {"status": "ok", "username": username, "avatar_url": row[1] or "/uploads/default.png", "bio": row[2] or "", "token": token}
...
def get_profile(username: str):
    conn = get_db()
    try:
        row = conn.execute("SELECT avatar_url, bio FROM users WHERE username = ?", (username,)).fetchone()
    finally:
        conn.close()
    if not row:
...
async def update_profile(token: str = Form(...), bio: str = Form(""), avatar: UploadFile = File(None)):
    username = require_auth(token)
    bio = bio[:150]
    conn = get_db()
    try:
        row = conn.execute("SELECT avatar_url FROM users WHERE username = ?", (username,)).fetchone()
        ...
        if avatar and avatar.filename:
            ...
            with open(file_path, "wb") as f:
                f.write(content)
            avatar_url = f"/uploads/{safe_name}"
        conn.execute("UPDATE users SET avatar_url = ?, bio = ? WHERE username = ?", (avatar_url, bio, username))
        conn.commit()
    finally:
        conn.close()
...
async def create_group(token: str = Form(...), title: str = Form(...), members: str = Form(...)):
    ...
    try:
        ...
        conn.execute("INSERT INTO groups (group_id, title, owner) VALUES (?, ?, ?)", (group_id, title, owner))
        for m in member_list:
...
def get_user_groups(username: str, token: str):
    ...
    conn = get_db()
    try:
        rows = conn.execute('''SELECT g.group_id, g.title, g.owner FROM groups g JOIN group_members gm ON g.group_id = gm.group_id WHERE gm.username = ?''', (username,)).fetchall()
    finally:
        conn.close()
...
def get_group_members(group_id: str, token: str):
    ...
    conn = get_db()
    try:
        row = conn.execute("SELECT owner FROM groups WHERE group_id = ?", (group_id,)).fetchone()
        if not row:
            raise HTTPException(status_code=404, detail="Группа не найдена")
        owner = row[0]
        members = [r[0] for r in conn.execute("SELECT username FROM group_members WHERE group_id = ?", (group_id,)).fetchall()]
    finally:
        conn.close()
...
async def leave_group(group_id: str, token: str = Form(...)):
    ...
    try:
        ...
        if not row:
            raise HTTPException(status_code=404, detail="Группа не найдена")
        conn.execute("DELETE FROM group_members WHERE group_id = ? AND username = ?", (group_id, username))
        remaining = [r[0] for r in conn.execute("SELECT username FROM group_members WHERE group_id = ?", (group_id,)).fetchall()]
        if not remaining:
            conn.execute("DELETE FROM groups WHERE group_id = ?", (group_id,))
            conn.execute("DELETE FROM messages WHERE chat_id = ?", (group_id,))
        elif row[0] == username:
            conn.execute("UPDATE groups SET owner = ? WHERE group_id = ?", (remaining[0], group_id))
        conn.commit()
    finally:
        conn.close()
    return {"status": "ok"}
    ...
    try:
        while True:
            ...
            if msg_type == "message":
                ...
                if is_group and not is_group_member(target, username): continue
                ...
                try:
                    cursor = conn.cursor()
                    cursor.execute("INSERT INTO messages (chat_id, sender, content, msg_type, status, reply_to) VALUES (?, ?, ?, ?, ?, ?)", (chat_id, username, content, content_type, initial_status, reply_to_id))
                    conn.commit()
                    msg_id = cursor.lastrowid
                    reply_preview = None
                    if reply_to_id:
                        rr = cursor.execute("SELECT sender, content, msg_type FROM messages WHERE id = ?", (reply_to_id,)).fetchone()
                        if rr:
                            reply_preview = {"id": reply_to_id, "sender": rr[0], "content": rr[1], "msg_type": rr[2]}
                finally:
                    conn.close()

                msg_payload = {"type": "message", "id": msg_id, "chat_id": chat_id, "target": target, "sender": username, "content": content, "msg_type": content_type, "status": initial_status, "is_group": is_group, "duration": duration, "reply_to": reply_preview, "reactions": []}
                ...
                if is_group:
                    for member_user in get_group_member_usernames(target):
                        ...
                        if not ok:
                            await asyncio.to_thread(send_push_to_user, member_user, f"👥 {username}", content[:100])
                else:
                    await manager.send_to_user(msg_payload, username)
                    if target != username:
                        ok = await manager.send_to_user(msg_payload, target)
                        if not ok:
                            await asyncio.to_thread(send_push_to_user, target, f"💬 {username}", content[:100])

            elif msg_type == "read_receipt":
                chat_id = data.get("chat_id")
                sender_to_notify = data.get("sender")
                if not chat_id or not sender_to_notify: continue
                conn = get_db()
                try:
                    conn.execute("UPDATE messages SET status = 'read' WHERE chat_id = ? AND sender = ? AND status != 'read'", (chat_id, sender_to_notify))
                    conn.commit()
                finally:
                    conn.close()
                await manager.send_to_user({"type": "status_update", "chat_id": chat_id, "status": "read"}, sender_to_notify)
                ...
                if not message_id or not target or not emoji: continue
                if is_group and not is_group_member(target, username): continue
                conn = get_db()
                try:
                    existing = conn.execute("SELECT emoji FROM reactions WHERE message_id = ? AND username = ?", (message_id, username)).fetchone()
                    if existing and existing[0] == emoji:
                        conn.execute("DELETE FROM reactions WHERE message_id = ? AND username = ?", (message_id, username))
                    else:
                        conn.execute("REPLACE INTO reactions (message_id, username, emoji) VALUES (?, ?, ?)", (message_id, username, emoji))
                    conn.commit()
                    reactions = [{"username": r[0], "emoji": r[1]} for r in conn.execute("SELECT username, emoji FROM reactions WHERE message_id = ?", (message_id,)).fetchall()]
                finally:
                    conn.close()
                ...
                if is_group and not is_group_member(target, username): continue
                typing_payload = {"type": "typing", "sender": username, "target": target, "is_group": is_group, "is_typing": is_typing}
                if is_group:
                    for member_user in get_group_member_usernames(target):
